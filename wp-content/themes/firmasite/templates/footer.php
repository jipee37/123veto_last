...
<footer id="footer" class="site-footer clearfix" role="contentinfo">
    <div class="site-info <?php echo $firmasite_settings["layout_container_class"]; ?>">
		...
		<?php if($firmasite_settings["poweredby"]) { ?>
        	<small id="firmasite_poweredby" class="text-muted generator"><a href="<?php echo esc_url( __( 'http://wordpress.org/', 'firmasite' ) ); ?>" title="<?php printf( esc_attr__( 'Proudly powered by %s', 'firmasite' ), 'WordPress' ); ?>"><i id="wplogo" class="icon-wordpress"></i></a></small>
        <?php } ?>
        <?php if($firmasite_settings["designer"]) { ?>
        	<small id="firmasite_designer" class="text-muted designer"><?php _e("Theme:",'firmasite'); ?> <a href="//firmasite.com/" class="text-muted" rel="designer">FirmaSite</a></small>
        <?php } ?>
    </div><!-- .site-info -->
