...
{action:"joinleave_group",cookie:bp_get_cookies(),gid:a,_wpnonce:b},function(a){var b=c.parent();jq("body.directory").length?jq(b).fadeOut(200,function(){b.fadeIn(200).html(a);var d=jq("#groups-personal span"),f=1;c.hasClass("leave-group")?(b.hasClass("hidden")&&b.closest("li").slideUp(200),f=0):c.hasClass("request-membership")&&(f=!1);d.length&&!1!==f&&(f?d.text((d.text()>>0)+1):d.text((d.text()>>0)-1))}):window.location.reload()});return!1});jq("#buddypress").on("click",".pending",function(){return!1});
...
!0}):jq(".message-check").each(function(){this.checked=!1})});jq("#messages-bulk-manage").attr("disabled","disabled");jq("#messages-select").on("change",function(){jq("#messages-bulk-manage").attr("disabled",0>=jq(this).val().length)});starAction=function(){var a=jq(this);jq.post(ajaxurl,{action:"messages_star",message_id:a.data("message-id"),star_status:a.data("star-status"),nonce:a.data("star-nonce"),bulk:a.data("star-bulk")},function(b){1===parseInt(b,10)&&("unstar"===a.data("star-status")?(a.data("star-status",
"star"),a.removeClass("message-action-unstar").addClass("message-action-star"),a.find(".bp-screen-reader-text").text(BP_PM_Star.strings.text_star),1===BP_PM_Star.is_single_thread?a.prop("title",BP_PM_Star.strings.title_star):a.prop("title",BP_PM_Star.strings.title_star_thread)):(a.data("star-status","unstar"),a.removeClass("message-action-star").addClass("message-action-unstar"),a.find(".bp-screen-reader-text").text(BP_PM_Star.strings.text_unstar),1===BP_PM_Star.is_single_thread?a.prop("title",BP_PM_Star.strings.title_unstar):
a.prop("title",BP_PM_Star.strings.title_unstar_thread)))});return!1};jq("#message-threads").on("click","td.thread-star a",starAction);jq("#message-thread").on("click",".message-star-actions a",starAction);jq("#message-threads td.bulk-select-check :checkbox").on("change",function(){var a=jq(this),b=a.closest("tr").find(".thread-star a");a.prop("checked")?"unstar"===b.data("star-status")?BP_PM_Star.star_counter++:BP_PM_Star.unstar_counter++:"unstar"===b.data("star-status")?BP_PM_Star.star_counter--:
BP_PM_Star.unstar_counter--;0<BP_PM_Star.star_counter&&0===parseInt(BP_PM_Star.unstar_counter,10)?jq('option[value="star"]').hide():jq('option[value="star"]').show();0<BP_PM_Star.unstar_counter&&0===parseInt(BP_PM_Star.star_counter,10)?jq('option[value="unstar"]').hide():jq('option[value="unstar"]').show()});jq("#select-all-notifications").click(function(a){this.checked?jq(".notification-check").each(function(){this.checked=!0}):jq(".notification-check").each(function(){this.checked=!1})});jq("#notification-bulk-manage").attr("disabled",
"disabled");jq("#notification-select").on("change",function(){jq("#notification-bulk-manage").attr("disabled",0>=jq(this).val().length)});jq("#close-notice").on("click",function(){jq(this).addClass("loading");jq("#sidebar div.error").remove();jq.post(ajaxurl,{action:"messages_close_notice",notice_id:jq(".notice").attr("rel").substr(2,jq(".notice").attr("rel").length)},function(a){jq("#close-notice").removeClass("loading");"-1"===a[0]+a[1]?(jq(".notice").prepend(a.substr(2,a.length)),jq("#sidebar div.error").hide().fadeIn(200)):
...
function bp_init_activity(){jq.cookie("bp-activity-oldestpage",1,{path:"/"});void 0!==jq.cookie("bp-activity-filter")&&jq("#activity-filter-select").length&&jq('#activity-filter-select select option[value="'+jq.cookie("bp-activity-filter")+'"]').prop("selected",!0);void 0!==jq.cookie("bp-activity-scope")&&jq(".activity-type-tabs").length&&(jq(".activity-type-tabs li").each(function(){jq(this).removeClass("selected active")}),jq("#activity-"+jq.cookie("bp-activity-scope")+", .item-list-tabs li.current").addClass("selected"))}
function bp_init_objects(c){jq(c).each(function(d){void 0!==jq.cookie("bp-"+c[d]+"-filter")&&jq("#"+c[d]+"-order-select select").length&&jq("#"+c[d]+'-order-select select option[value="'+jq.cookie("bp-"+c[d]+"-filter")+'"]').prop("selected",!0);void 0!==jq.cookie("bp-"+c[d]+"-scope")&&jq("div."+c[d]).length&&(jq(".item-list-tabs li").each(function(){jq(this).removeClass("selected active")}),jq("#"+c[d]+"-"+jq.cookie("bp-"+c[d]+"-scope")+", #object-nav li.current").addClass("selected"))})}
...
!0);jq(".widget_bp_activity_widget h2 span.ajax-loader").show();bp_ajax_request&&bp_ajax_request.abort();bp_ajax_request=jq.post(ajaxurl,{action:"activity_widget_filter",cookie:bp_get_cookies(),_wpnonce_activity_filter:jq("#_wpnonce_activity_filter").val(),scope:c,filter:d},function(c){jq(".widget_bp_activity_widget h2 span.ajax-loader").hide();jq("div.activity").fadeOut(100,function(){jq(this).html(c.contents);jq(this).fadeIn(100);bp_legacy_theme_hide_comments()});void 0!==c.feed_url&&jq(".directory #subnav li.feed a, .home-page #subnav li.feed a").attr("href",
