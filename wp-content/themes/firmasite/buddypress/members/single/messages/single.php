...
<div id="message-thread">
	...
	<?php if ( bp_thread_has_messages() ) : ?>
		...
		<p id="message-recipients">
			<span class="highlight">
				...
				<?php elseif ( bp_get_max_thread_recipients_to_list() <= bp_get_thread_recipients_count() ) : ?>
					...
					<?php printf( __( 'Conversation between %s recipients.', 'firmasite' ), number_format_i18n( bp_get_thread_recipients_count() ) ); ?>

				<?php else : ?>

					<?php printf( __( 'Conversation between %s and you.', 'firmasite' ), bp_get_thread_recipients_list() ); ?>

				<?php endif; ?>

			</span>

			<a class="btn btn-default btn-xs button confirm" href="<?php bp_the_thread_delete_link(); ?>" title="<?php esc_attr_e( "Delete Conversation", 'firmasite' ); ?>"><?php _e( 'Delete', 'firmasite' ); ?></a>
		</p>
		...
		<?php while ( bp_thread_messages() ) : bp_thread_the_message(); ?>
			...
			<div class="message-box <?php bp_the_thread_message_css_class(); ?>">
				...
				<div class="message-metadata">
					...
					<span class="label label-default activity"><?php bp_the_thread_message_time_since(); ?></span>
		...
		<form id="send-reply" action="<?php bp_messages_form_action(); ?>" method="post" class="standard-form">
			...
			<div class="message-box well well-sm">
				...
				<div class="message-content">
					...
					*/
					...
					<div class="submit">
						<input type="submit" class="btn btn-primary" name="send" value="<?php esc_attr_e( 'Send Reply', 'firmasite' ); ?>" id="send_reply_button"/>
					</div>
