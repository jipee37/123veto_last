...
<?php if ( bp_has_message_threads( bp_ajax_querystring( 'messages' ) ) ) : ?>
	...
	<form action="<?php echo bp_loggedin_user_domain() . bp_get_messages_slug() . '/' . bp_current_action() ?>/bulk-manage/" method="post" id="messages-bulk-management" class="margin-bot">
	...
	<div class="modal firmasite-modal-static"><div class="modal-dialog"><div class="modal-content"><div class="modal-body">
		<table id="message-threads" class="messages-notices">
			...
			<thead>
				<tr>
					<th scope="col" class="thread-checkbox bulk-select-all"><label class="sr-only bp-screen-reader-text" for="select-all-messages"><?php _e( 'Select all', 'firmasite' ); ?></label><input id="select-all-messages" type="checkbox"></th>
					...
					<?php if ( bp_is_active( 'messages', 'star' ) ) : ?>
						<th scope="col" class="thread-star"><span class="message-action-star"><span class="icon"></span> <span class="screen-reader-text"><?php _e( 'Star', 'firmasite' ); ?></span></span></th>
					<?php endif; ?>
				...
				<?php while ( bp_message_threads() ) : bp_message_thread(); ?>
					...
					<tr id="m-<?php bp_message_thread_id(); ?>" class="<?php bp_message_css_class(); ?><?php if ( bp_message_thread_has_unread() ) : ?> unread<?php else: ?> read<?php endif; ?>">
						...
						<td class="thread-info">
							<p><a href="<?php bp_message_thread_view_link(); ?>" title="<?php esc_attr_e( "View Message", 'firmasite' ); ?>"><?php bp_message_thread_subject(); ?></a></p>
							<p class="thread-excerpt"><?php bp_message_thread_excerpt(); ?></p>
						</td>

						<?php if ( 'sentbox' != bp_current_action() ) : ?>
							<td class="thread-from text-muted">
