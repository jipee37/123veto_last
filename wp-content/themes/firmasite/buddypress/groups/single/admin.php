...
<form action="<?php bp_group_admin_form_action(); ?>" name="group-settings-form" id="group-settings-form" class="standard-form margin-top" method="post" enctype="multipart/form-data">
...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
	...
	<?php $content = bp_get_group_description_editable(); 
    ...
    */
	...
	<p>
		<label for="group-notify-members">
			<input type="checkbox" name="group-notify-members" value="1" /> <?php _e( 'Notify group members of these changes via email', 'firmasite' ); ?>
		</label>
	</p>
	...
	<p><input type="submit" class="btn btn-primary" value="<?php esc_attr_e( 'Save Changes', 'firmasite' ); ?>" id="save" name="save" /></p>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
	...
	<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<?php if ( bp_get_group_has_avatar() ) : ?>
				...
				<p><?php _e( "If you'd like to remove the existing group profile photo but not upload a new one, please use the delete group profile photo button.", 'firmasite' ); ?></p>

				<?php bp_button( array( 'id' => 'delete_group_avatar', 'component' => 'groups', 'wrapper_id' => 'delete-group-avatar-button', 'link_class' => 'edit btn btn-default', 'link_href' => bp_get_group_avatar_delete_link(), 'link_title' => __( 'Delete Group Profile Photo', 'firmasite' ), 'link_text' => __( 'Delete Group Profile Photo', 'firmasite' ) ) ); ?>
	...
	<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
		...
		<h4 class="page-header"><?php _e( 'Crop Profile Photo', 'firmasite' ); ?></h4>
...
<?php if ( bp_is_group_admin_screen( 'manage-members' ) ) : ?>
	...
	<div class="bp-widget">
		...
		<ul id="admins-list" class="item-list single-line list-unstyled">
			...
			<li>
				...
				<h5>
					...
					<span class="small">
						<a class="btn btn-default btn-xs button confirm admin-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'firmasite' ); ?></a>
					</span>
					<?php endif; ?>
				</h5>
			</li>
	...
	<?php if ( bp_group_has_moderators() ) : ?>
		<div class="bp-widget">
			...
			<?php if ( bp_has_members( '&include=' . bp_group_mod_ids() ) ) : ?>
				<ul id="mods-list" class="item-list single-line list-unstyled">
					...
					<li>
						...
						<h5>
							<a href="<?php bp_member_permalink(); ?>"> <?php bp_member_name(); ?></a>
							<span class="small">
								<a href="<?php bp_group_member_promote_admin_link( array( 'user_id' => bp_get_member_user_id() ) ); ?>" class="btn btn-default btn-xs button confirm mod-promote-to-admin" title="<?php esc_attr_e( 'Promote to Admin', 'firmasite' ); ?>"><?php _e( 'Promote to Admin', 'firmasite' ); ?></a>
								<a class="btn btn-default btn-xs button confirm mod-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'firmasite' ); ?></a>
							</span>
						</h5>
					</li>
					<?php endwhile; ?>
	...
	<div class="bp-widget">
		...
		<?php if ( bp_group_has_members( 'per_page=15&exclude_banned=0' ) ) : ?>
			...
			<ul id="members-list" class="item-list single-line list-unstyled">
                     ...
                     <div class="panel panel-default"><div class="panel-body">
						...
						<h5>
							...
							<?php if ( bp_get_group_member_is_banned() ) : ?>
								...
								<a href="<?php bp_group_member_unban_link(); ?>" class="btn btn-default btn-xs button confirm member-unban" title="<?php esc_attr_e( 'Unban this member', 'firmasite' ); ?>"><?php _e( 'Remove Ban', 'firmasite' ); ?></a>

							<?php else : ?>

								<a href="<?php bp_group_member_ban_link(); ?>" class="btn btn-default btn-xs button confirm member-ban" title="<?php esc_attr_e( 'Kick and ban this member', 'firmasite' ); ?>"><?php _e( 'Kick &amp; Ban', 'firmasite' ); ?></a>
								<a href="<?php bp_group_member_promote_mod_link(); ?>" class="btn btn-default btn-xs button confirm member-promote-to-mod" title="<?php esc_attr_e( 'Promote to Mod', 'firmasite' ); ?>"><?php _e( 'Promote to Mod', 'firmasite' ); ?></a>
								<a href="<?php bp_group_member_promote_admin_link(); ?>" class="btn btn-default btn-xs button confirm member-promote-to-admin" title="<?php esc_attr_e( 'Promote to Admin', 'firmasite' ); ?>"><?php _e( 'Promote to Admin', 'firmasite' ); ?></a>

							<?php endif; ?>

								<a href="<?php bp_group_member_remove_link(); ?>" class="btn btn-default btn-xs button confirm" title="<?php esc_attr_e( 'Remove this member', 'firmasite' ); ?>"><?php _e( 'Remove from group', 'firmasite' ); ?></a>
...
<?php if ( bp_is_group_admin_screen( 'delete-group' ) ) : ?>
	...
	<div class="submit">
		<input type="submit" class="btn btn-primary" disabled="disabled" value="<?php esc_attr_e( 'Delete Group', 'firmasite' ); ?>" id="delete-group-button" name="delete-group-button" />
	</div>
