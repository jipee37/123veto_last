...
<div id="buddypress">
	...
	<form action="" method="post" id="forums-directory-form" class="dir-form">
		...
		<div id="forums-dir-list" class="forums dir-list margin-top">
		...
		<?php if ( is_user_logged_in() ) : ?>
			...
			<?php if ( bp_is_active( 'groups' ) && bp_has_groups( 'user_id=' . bp_loggedin_user_id() . '&type=alphabetical&max=100&per_page=100' ) ) : ?>
				...
				<form action="" method="post" id="forum-topic-form" class="standard-form">
					...
					<select class="form-control input-sm" id="topic_group_id" name="topic_group_id">
					...
					<div class="submit">
						...
						<input type="button" name="submit_topic_cancel" id="submit_topic_cancel" value="<?php esc_attr_e( 'Cancel', 'firmasite' ); ?>" />
					</div>
			...
			<?php elseif ( bp_is_active( 'groups' ) ) : ?>
				...
				<div class="clearfix"></div><div id="message" class="info alert alert-info">
					...
					<p><?php printf( __( "You are not a member of any groups so you don't have any group forums you can post in. To start posting, first find a group that matches the topic subject you'd like to start. If this group does not exist, why not <a href='%s'>create a new group</a>? Once you have joined or created the group you can post your topic in that group's forum.", 'firmasite' ), trailingslashit( bp_get_groups_directory_permalink() . 'create' ) ); ?></p>
