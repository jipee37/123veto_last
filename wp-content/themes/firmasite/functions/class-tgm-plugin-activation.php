		...
		public function notices() {
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					$show_update_link  = $update_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, $this->domain ) . '</a>' : '';
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				elseif ( version_compare( $installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<' ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Updated', TGM_Plugin_Activation::$instance->domain ) );
				else
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Active', TGM_Plugin_Activation::$instance->domain ) );
