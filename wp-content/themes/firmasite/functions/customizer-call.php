...
// We are calling this early for some customize_register settings and admin panel but re-running in wp action as main.
add_action( 'after_setup_theme', 'firmasite_settings_init', 9);
add_action( 'wp', 'firmasite_settings_init', 9);

function firmasite_settings_init() {
	
	global $firmasite_settings, $firmasite_settings_core;
	$firmasite_settings = $firmasite_settings_core = get_option( "firmasite_settings", array() ); // site options
 ...
 * You can use multi subsets with comma seperated. Example: 'cyrillic,cyrillic-ext'
