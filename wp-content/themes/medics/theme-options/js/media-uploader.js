(function($) {
	$(document).ready(function() {
		...
		function medics_add_file(event, selector) {
			...
			frame.on( 'select', function() {
				...
				selector.find('.remove-image, .remove-file').on('click', function() {
					medics_remove_file( $(this).parents('.section') );
				});
			});
		...
		function medics_remove_file(selector) {
			...
			selector.find('.upload-button').live('click', function() {
				medics_add_file(event, $(this).parents('.section'));
			});
		}
